...
void LDA::initialize(){
	...
	for (k=0; k < num_topic; k++){
		...
		for (v=0; v < num_vocab; v++){
			// lam[k][v] = beta[v] + num_topic/num_vocab;
			lam[k][v] = (double) rnd();
		}
	...
	for (j=0; j < num_lesion; j++){
		...
		for (k=0; k < num_topic; k++){
			// tau[j][k] = alpha[k] + num_lesion/num_topic;
			tau[j][k] = (double) rnd();
		}
...
void LDA::Update_phi(){
	...
	// for(j=0; j < num_lesion; j++){
		// for(d=0; d < documents[j].size(); d++){
			// for(i=0; i < documents[j][d].size(); i++){
				// for(k=0; k < num_topic; k++){
					// cout<<"phi["<<j<<"]["<<d<<"]["<<i<<"]["<<k<<"]: "<<phi[j][d][i][k]<<endl;
				// }
			// }
		// }
	// }
...
void LDA::Update_rho(){
	...
	vector<vector<vector<vector<double> > > > temp_rho;
	temp_rho.resize(num_lesion);
	for(j=0; j < num_lesion; j++){
		temp_rho[j].resize(documents[j].size());
		for (d=0; d < documents[j].size(); d++){
			temp_rho[j][d].resize(num_topic);
			for (k=0; k < num_topic; k++){
				temp_rho[j][d][k].resize(documents[j][d].size(), 0);
			}
			for (i=0; i < documents[j][d].size(); i++){
				for (k=0; k < num_topic; k++){
					temp_rho[j][d][k][i] = phi[j][d][i][k];
				}
			}
		}
	}
	...
	for(j=0; j < num_lesion; j++){
		...
		for(d=0; d < documents[j].size(); d++){
			for(k=0; k < num_topic; k++){
				rho[j][d][k] = eta[k] * tau[j][k] / sum_tau[j] + accumulate(temp_rho[j][d][k].begin(), temp_rho[j][d][k].end(), 0.0);
				// rho[j][d][k] = eta[k] * tau[j][k] / sum_tau[j] + exp(log_sum_exp(temp_rho[j][d][k], doc_length));
			}
		}
	}
	...
	// for(j=0; j < num_lesion; j++){
		// for (d=0; d < documents[j].size(); d++){
			// for(k=0; k < num_topic; k++){
				// cout<<"rho["<<j<<"]["<<d<<"][0]["<<k<<"]: "<<rho[j][d][k]<<endl;
			// }
		// }
	// }
...
void LDA::Update_lam(){
	...
	// vector<vector<vector<double> > > temp_phi;
    // temp_phi.resize(num_topic);
    // for (k=0; k < num_topic; k++){
        // temp_phi[k].resize(num_vocab);
		// for (j=0; j < num_lesion; j++){
			// for (d=0; d < documents[j].size(); d++){
				// for (i=0; i < documents[j][d].size(); i++){
					// temp_phi[k][documents[j][d][i]].push_back(phi[j][d][i][k]);
				// }
			// }
		// }
    // }
	...
	for (k=0; k < num_topic; k++){
        for (v=0; v < num_vocab; v++){
			// int vec_size = temp_phi[k][v].size();
            // lam[k][v] = beta[v] + exp(log_sum_exp(temp_phi[k][v], vec_size));;
...
void LDA::Update_tau(){
	...
	// vector<vector<vector<double> > > temp_sum_rho;
    // temp_sum_rho.resize(num_lesion);
    // for(j=0; j < num_lesion; j++){
        // temp_sum_rho[j].resize(num_topic);
        // for (k=0; k < num_topic; k++){
            // temp_sum_rho[j][k].resize(documents[j].size(), 0);
        // }
        // for(d=0; d < documents[j].size(); d++){
            // for (k=0; k < num_topic; k++){
                // temp_sum_rho[j][k][d] = rho[j][d][k];
            // }
        // }
    // }
    ...
    for(j=0; j < num_lesion; j++){
		...
		for(k=0; k < num_topic; k++){
			for(d=0; d < documents[j].size(); d++){
				// sum_rho[j][k] += boost::math::digamma(temp_sum_rho[j][k][d]);
        ...
        for (j=0; j < num_lesion; j++){
			for(d=0; d < documents[j].size(); d++){
				if(tau[j][k]>0)
					...
					new_numerator_eta[k]
						+= ( -1 + boost::math::digamma( rho[j][d][k] - tau[j][k] + eta[k] * tau[j][k] / sum_tau[j])
							- boost::math::digamma(eta[k] * tau[j][k] / sum_tau[j] )) * eta[k];
        ...
        for (j=0; j < num_lesion; j++){
			for(d=0; d < documents[j].size(); d++){
				if(sum_tau[j]>0)
					...
					new_denominator_eta[k]
						+= -1 + boost::math::digamma( sum_rho[j][d] - sum_tau[j]  + sum_eta )
						- boost::math::digamma(sum_eta);
			}
        }
    }
	...
	// for(j=0; j < num_lesion; j++){
		// for(d=0; d < documents[j].size(); d++){
			// for(k=0; k < num_topic; k++){
				...
				// }
				// else{
					// sum -= (tau[j][k]/sum_tau[j])*( boost::math::digamma(eta[k]*tau[j][k]/sum_tau[j]) - (-1 - boost::math::digamma(tau[j][k]) + boost::math::digamma(rho[j][d][k]) ) );
				// }
	...
	// for(k=0; k < num_topic; k++){
		// for(j=0; j < num_lesion; j++){
			...
			// for(d=0; d < documents[j].size(); d++){
				...
				// }
				// else{
					// g_eta[k] += -1* ( -1 - boost::math::digamma(sum_tau[j]) + boost::math::digamma(sum_rho[j][d]) );
				// }
...
void LDA::calc_vlb(){
		...
		for(d=0; d < documents[j].size(); d++){
			for (k=0; k < num_topic; k++){
				if( tau[j][k] > 0 && sum_tau[j] > 0 ){
					third_comp += boost::math::lgamma(eta[k]) - (eta[k]/sum_tau[j])*(num_topic-1) - (eta[k]- num_topic) * (log(sum_tau[j]) - boost::math::digamma(sum_tau[j]) + boost::math::digamma(sum_rho[j][d])) - (boost::math::lgamma(eta[k]*tau[j][k]/sum_tau[j]) + (1-eta[k]*tau[j][k]/sum_tau[j]) * (log(tau[j][k]) - boost::math::digamma(tau[j][k]) + boost::math::digamma(rho[j][d][k])));
				}
				else{
					if( sum_tau[j] > 0 ){
						third_comp += boost::math::lgamma(eta[k]) - (eta[k]/sum_tau[j])*(num_topic-1) - (eta[k]- num_topic) * (log(sum_tau[j]) - boost::math::digamma(sum_tau[j]) + boost::math::digamma(sum_rho[j][d])) - (boost::math::lgamma(eta[k]*tau[j][k]/sum_tau[j]) + (1-eta[k]*tau[j][k]/sum_tau[j]) * (-1 - boost::math::digamma(tau[j][k]) + boost::math::digamma(rho[j][d][k])));
					}
					else{
						if(tau[j][k] > 0){
							third_comp += boost::math::lgamma(eta[k]) - (eta[k]/sum_tau[j])*(num_topic-1) - (eta[k]- num_topic) * (-1 - boost::math::digamma(sum_tau[j]) + boost::math::digamma(sum_rho[j][d])) - (boost::math::lgamma(eta[k]*tau[j][k]/sum_tau[j]) + (1-eta[k]*tau[j][k]/sum_tau[j]) * (log(tau[j][k]) - boost::math::digamma(tau[j][k]) + boost::math::digamma(rho[j][d][k])));
						}
						else{
							third_comp += boost::math::lgamma(eta[k]) - (eta[k]/sum_tau[j])*(num_topic-1) - (eta[k]- num_topic) * (-1 - boost::math::digamma(sum_tau[j]) + boost::math::digamma(sum_rho[j][d])) - (boost::math::lgamma(eta[k]*tau[j][k]/sum_tau[j]) + (1-eta[k]*tau[j][k]/sum_tau[j]) * (-1 - boost::math::digamma(tau[j][k]) + boost::math::digamma(rho[j][d][k])));
						}
					}
	...
	for (j=0; j < num_lesion; j++){
		for(d=0; d < documents[j].size(); d++){
			for(i=0; i < documents[j][d].size(); i++){
				for (k=0; k < num_topic; k++){
					// fourth_comp += exp(phi[j][d][i][k]) * (boost::math::digamma(rho[j][d][k]) - boost::math::digamma(sum_rho[j][d]));
	...
	for (j=0; j < num_lesion; j++){
		for(d=0; d < documents[j].size(); d++){
			for(i=0; i < documents[j][d].size(); i++){
				for (k=0; k < num_topic; k++){
					// fifth_comp += exp(phi[j][d][i][k]) * (boost::math::digamma(lam[k][documents[j][d][i]]) - boost::math::digamma(sum_lam[k]));
	...
	for (j=0; j < num_lesion; j++){
		for(d=0; d < documents[j].size(); d++){
			for(i=0; i < documents[j][d].size(); i++){
				for (k=0; k < num_topic; k++){
					// if( isnan(phi[j][d][i][k] * log(phi[j][d][i][k])))
						// cout<<"phi["<<j<<"]["<<d<<"]["<<i<<"]["<<k<<"]: "<<phi[j][d][i][k]<<endl;
					
					if( phi[j][d][i][k] > 0){
						// ninth_comp += exp(phi[j][d][i][k]) * phi[j][d][i][k];
...
void LDA::load_data(){
	...
	for(int le=0; le < num_lesion; le++){
		ifstream ifs;
		string input_file_name = "D:/mutational_signature/MS_cLDA/data/data" + to_string(data_type) + "_o"
...
void LDA::write_data(){
    ofstream ofs;
    string output_file_name = "D:/mutational_signature/MS_cLDA/result/data" + to_string(data_type) + "_o" +
